(function (window) {
     ...
     * @returns {Promise<any>}
     */
    const loadClientLanguages = ({baseUrl, path}, config = {}) => {
     ...
     * @returns {Promise<*>}
     */
    const loadLanguageConfig = ({baseUrl, path, language}, config = {}) => {
     ...
     * 初始化配置数据
     */
    const useLanguageOptions = (openAPI, apiTags, errorRef) => {
        const languages = ref([]);
